# -*- coding: iso-8859-1 -*-
"""
    PageComment2.py  Version 0.992  February , 2008
    ...
    Notes
        ...
        - This version only works with MoinMoin version 1.6.1 and higher (until broken)
          No new functionality was included in the 0.99x version
...
import MoinMoin.macro  # keep full name for scope clarity
...
def execute(macro, args):
    ...
    if action == addcommand:
        ...
        form_fields = {'comicon': Globs.defaulticon, 'comauthor': '', 'comtext': '', 'compasswd': '', 'comrev': 0, 'autopasswd': '', 'button_save': '', 'button_preview': '', 'commarkup%d' % Globs.formid: '0'}
        ...
        if not len(missingfields) == len(required_fields):
            if not missingfields:
                ...
                # check input
                if comicon and (not comicon in config.smileys):
                ...
                ## PREVIEW
                    ...
                    commentpreview = previewcomment(comicon, comauthor, comtext, commarkup)
                
                ## ADD
                elif btnsave:
                    flag = addcomment(macro, comicon, comauthor, comtext, compasswd, comrev, comautopass, commarkup)
    ...
    commentformhtml = commentformsection(comauthor, comtext, compasswd, comicon, comrev, comautopass, commarkup)
...
def commentformsection(comauthor, comtext, compasswd, comicon, comrev, autopass, commarkup):
    html = []
...
def getparams(args):
    ...
    try:
        Params.nosmiley = int(params.get('nosmiley', 0))
    except ValueError:
        Params.nosmiley = 0

    try:
        Params.rows = int(params.get('rows', 4))
    except ValueError:
        Params.rows = 4
...
def setglobalvalues(macro):
    ...
    if Params.markup:
        ...
        macronames = [name for name in MoinMoin.macro.getNames(request.cfg)\
                           if name not in Globs.macroallowed]
...
//-->
    ...
    html2 = [
        ...
        u'<form action="%s#pagecomment%d" name="comment" METHOD="POST" %s>' % (page_url, Globs.formid, onSubmitCode),
        ...
        u'%s>%s</textarea></td>' % (scripthtml, wikiutil.escape(tmptext)),
        u'<td style="%s vertical-align: bottom;"><input type="submit" name="button_save" value="%s" style="font-size: 9pt; width: 6em; height:3em; ">%s</td>' % (cellstyle, _('Save'), previewbutton),
        u'</tr>',
...
def previewcomment(comicon, comauthor, comtext, commarkup):
    request = Globs.macro.request
    _ = request.getText
    cfg = request.cfg
    ...
    htmlcommentitem = u'\n'.join(htmlcomment) % {
        ...
        'date': time.strftime(cfg.datetime_fmt, time.localtime(time.time())),
        'delform': ''
        }
