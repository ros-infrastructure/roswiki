...
def distro_html(distro, loaded_distro_names):
    ...
    html = (
        ...
        '''onClick="Version(%s);this.style.color='#e6e6e6';this.style.background='#3e4f6e';''' % (distro, sectionarg)
    )
    for inactive_distro in inactive:
        html += (
            '''document.getElementById('%s').style.background='#e6e6e6';'''
            '''document.getElementById('%s').style.color='#3e4f6e';''' % (inactive_distro, inactive_distro)
        )
